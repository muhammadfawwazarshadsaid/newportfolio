...
// <<< Gunakan tipe langsung dari file types >>>
...
export function SettingsDialog() {
  ...
  const [isLoading, setIsLoading] = React.useState(true); // Mulai true saat dialog belum dibuka
  const [error, setError] = React.useState<string | null>(null);
  // --- End State ---

  const supabase = createClient(); // Buat instance client sekali

  // --- Data Fetching dengan useEffect (Dengan Order) ---
  React.useEffect(() => {
    const fetchPortfolioData = async () => {
      ...
      try {
        // 1. Fetch Kategori - ORDER BY "order", name
        const { data: categories, error: categoriesError } = await supabase
          .from('categories')
          .select('*')
          .order('order', { ascending: true, nullsFirst: false }) // <<< UTAMA: Urutkan berdasarkan 'order'
          .order('name', { ascending: true });                   // <<< SEKUNDER: Urutkan berdasarkan nama

        if (categoriesError) throw categoriesError;
        if (!categories) throw new Error("Categories not found.");

        // 2. Fetch Semua Proyek - ORDER BY "order", created_at DESC
        const { data: projects, error: projectsError } = await supabase
          .from('projects')
          .select('*') // Ambil semua kolom termasuk category_id
          .order('order', { ascending: true, nullsFirst: false }) // <<< UTAMA: Urutkan berdasarkan 'order' proyek
          .order('created_at', { ascending: false });             // <<< SEKUNDER: Urutkan berdasarkan terbaru

        if (projectsError) throw projectsError;
        if (!projects) throw new Error("Projects not found.");

        // 3. Transformasi Data (Urutan proyek akan mengikuti urutan fetch `projects`)
        const transformedData = categories.map((category: CategoryFromDB) => {
          // Filter proyek untuk kategori ini
          const categoryProjects = projects
            .filter((p: ProjectFromDB) => p.category_id === category.id)
            // Proyek sudah terurut sesuai fetch `projects` di atas
            .map((p: ProjectFromDB): ProjectDetail => ({
              title: p.title,
          ...
          return {
            ...
            details: categoryProjects, // list proyek sudah terurut
          };
        });

        setPortfolioNavData(transformedData); // Data sudah terurut

        // Set menu aktif awal jika data ada DAN belum ada menu aktif
        if (transformedData.length > 0 && activeMenu === null) { // Cek jika activeMenu masih null
           setActiveMenu(transformedData[0].name);
        } else if (transformedData.length === 0) {
           setActiveMenu(null); // Kosongkan jika tidak ada kategori
        }
        // Jika activeMenu sudah ada (misal dari state sebelumnya), biarkan saja

      } catch (err: any) {
        console.error("Error fetching portfolio data:", err);
        setError(err.message || "Gagal memuat data portofolio.");
        setPortfolioNavData([]); // Kosongkan data jika error
        setActiveMenu(null);
      } finally {
    ...
    // Hanya fetch jika dialog terbuka
    if (open) {
       fetchPortfolioData();
    } else {
        // Opsional: Reset state saat dialog ditutup jika diinginkan
        // setIsLoading(true);
        // setActiveMenu(null);
        // setPortfolioNavData([]);
    }
  ...
  }, [open, supabase]); // Re-fetch hanya jika 'open' berubah (atau supabase client)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="overflow-hidden p-0 rounded-lg md:max-h-[550px] md:max-w-[700px] lg:max-w-[850px] flex flex-col h-screen md:h-auto">
        ...
        {isLoading && ( <div className="flex items-center justify-center h-[400px] md:h-[550px]"> <p>Loading portfolio...</p> </div> )}
             ...
             <>
                {isMobile && (
                  <motion.div className="flex h-full flex-1 flex-col overflow-hidden" variants={dialogContainerVariants} initial="hidden" animate="visible">
                    ...
                    {!selectedDetail && activeMenu && ( // Tambah cek activeMenu
                      <motion.div className="px-4 pt-4 pb-2 border-b relative" variants={dialogItemVariants}>
                         ...
                         <div ref={scrollContainerRef} className="overflow-x-auto whitespace-nowrap pb-2 scrollbar-hide">
                            {/* Pastikan value Tabs sesuai dengan activeMenu */}
                            <Tabs value={activeMenu} className="w-full inline-block" onValueChange={handleMenuClick}>
                               <TabsList className="flex justify-start">
                                 {/* Map dari portfolioNavData yang sudah diurutkan */}
